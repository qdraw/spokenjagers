...
db.query('CREATE TABLE IF NOT EXISTS users (id MEDIUMINT NOT NULL AUTO_INCREMENT PRIMARY KEY, userid TEXT, displayname TEXT, health INTEGER, score INTEGER, money INTEGER, useragent TEXT, value TEXT)',
function(err, result){
...
function updatePoints (type,addition,userid) {

    var refreshIntervalId = setInterval(function () {
        if (global["checkIfUserExist_" + userid]) {
            
            global["money"][userid] = Number(global["money"][userid] + addition);

            if (type === "score" || type === "health" || type === "money" || type === "useragent") {
                db.query("UPDATE users SET "+ type +" = '" + global["money"][userid] + "' WHERE userid = '" + userid +"'");
...
function sendScore (socket,userid) {
